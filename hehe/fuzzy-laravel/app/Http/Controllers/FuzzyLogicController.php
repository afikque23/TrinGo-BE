...
class FuzzyLogicController extends Controller
{
    ...
    // ─── API: Ambil komponen + config per jenis motor ─────────────────────────
    ...
    // ─── API: Simpan konfigurasi threshold + variabel aktif ──────────────────
    ...
    // ─── API: Simpan membership function ─────────────────────────────────────
    ...
    public function storeComponent(Request $request)
    {
        ...
        DB::transaction(function () use ($validated) {
            foreach ($validated['motor_type_ids'] as $motorTypeId) {
                ...
                // Buat default MF untuk tiap variabel
                $defaults = [
    ...
    // ─── API: Toggle aktif/nonaktif komponen ─────────────────────────────────
    ...
    public function toggleComponent(ComponentConfig $component)
    {
        ...
        $this->log($component, 'Status Aktif', !$component->is_active ? 'aktif' : 'nonaktif', $status);
    ...
    {
        $name = $component->name;
        $component->delete(); // cascade ke fuzzy_variables dan fuzzy_rules

        return response()->json(['message' => "Komponen \"{$name}\" dihapus permanen"]);
    }

    // ─── API: Test kalkulasi Fuzzy ────────────────────────────────────────────
    ...
    // ─── API: Preview chart membership function ───────────────────────────────
    ...
    // ─── Helper: Format MF dari koleksi FuzzyVariable ────────────────────────
