...
{
    // Default MF values per variabel
    ...
    public function run(): void
    {
        ...
        foreach (['matic'=>'Matic','manual'=>'Manual/Bebek','sport'=>'Sport','adventure'=>'Adventure'] as $slug=>$name) {
            $types[$slug] = MotorType::firstOrCreate(['slug'=>$slug], ['name'=>$name]);
        }

        // 2. Buat komponen + MF + rules
        foreach ($this->components as $typeSlug => $comps) {
            ...
            foreach ($comps as $comp) {
                ...
                // 3. Buat MF untuk setiap variabel
                ...
                if ($component->fuzzyRules()->count() === 0) {
                    ...
                    foreach ($defaultRules as $r) {
                        FuzzyRule::create([
                            ...
                            'var1'     => $r['var1'],   'label1'   => $r['label1'],
                            'operator' => $r['op'],
                            'var2'     => $r['var2'],   'label2'   => $r['label2'],
                            'output'   => $r['output'], 'weight'   => $r['weight'],
                        ]);
                    }
                }
            }
        ...
        $this->command->info('✓ Fuzzy Logic seeded: 4 motor types, ' . array_sum(array_map('count', $this->components)) . ' components');
    }
}
