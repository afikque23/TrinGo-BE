...
class FuzzyLogicSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach (['matic'=>'Matic','manual'=>'Manual/Bebek','sport'=>'Sport','adventure'=>'Adventure'] as $slug=>$name) {
            $typeMap[$slug] = MotorType::firstOrCreate(['slug'=>$slug], ['name'=>$name, 'is_active'=>true]);
        }

        foreach ($this->components as $slug => $comps) {
            $motorType = $typeMap[$slug];
            ...
            foreach ($comps as $comp) {
                ...
                // Default rules (6 rule per komponen)
                if ($component->fuzzyRules()->count() === 0) {
                    $v1 = $comp['vars'][0];
                    $v2 = $comp['vars'][1] ?? $comp['vars'][0];

                    $rules = [
                        [$v1,'high',  'AND',$v2,'high',   'Kritis',       1.0],
                        [$v1,'high',  'AND',$v2,'medium', 'Kritis',       0.8],
                        [$v1,'medium','AND',$v2,'high',   'Perlu Servis', 0.9],
                        [$v1,'medium','AND',$v2,'medium', 'Perlu Servis', 1.0],
                        [$v1,'low',   'AND',$v2,'medium', 'Baik',         0.7],
                        [$v1,'low',   'AND',$v2,'low',    'Baik',         1.0],
                    ];

                    foreach ($rules as [$var1,$l1,$op,$var2,$l2,$out,$w]) {
                        FuzzyRule::create([
                            'component_config_id'=>$component->id,
                            'var1'=>$var1, 'label1'=>$l1, 'operator'=>$op,
                            'var2'=>$var2, 'label2'=>$l2, 'output'=>$out, 'weight'=>$w,
                        ]);
                    }
                }
            }
        ...
        $this->command->info("✓ Seeded: 4 motor types, {$total} components, " . ($total * 6) . " rules");
    }
}
