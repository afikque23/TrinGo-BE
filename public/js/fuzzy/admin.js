...
const ROUTES = {
    components: (motorTypeId) =>
        `/admin/fuzzy/components?motor_type_id=${motorTypeId}`,
...
function fuzzyAdmin() {
    return {
        ...
        addCompForm: {
            name: "",
            motorTypes: ["matic", "manual", "sport", "adventure"],
            warn: 1500,
            critical: 2500,
            resetInterval: 90,
            activeVars: ["jarak"],
        ...
        async switchTab(motorTypeId) {
            ...
            if (!this.components[motorTypeId]) {
                ...
                const first = this.components[motorTypeId].find(
                    (c) => c.is_active,
                );
        ...
        resetThreshold() {
            this.form.warn = 1500;
            this.form.critical = 2500;
            this.form.resetInterval = 90;
            this.hasChanges = true;
        },
        ...
        async saveAll() {
            ...
            for (const varKey of this.form.activeVars) {
                ...
                await this.api(
                    "PATCH",
                    ROUTES.membership(this.selectedComp.id),
                    {
                        var_key: varKey,
                        low_a: mf.low[0],
                        low_b: mf.low[1],
        ...
        async saveRule() {
            if (this.editRule) {
                const res = await this.api(
                    "PATCH",
                    ROUTES.rulesUpdate(this.editRule.id),
                    this.ruleForm,
                );
                const rules = this.selectedComp.rules.map((r) =>
                    r.id === this.editRule.id ? res.rule : r,
                );
                this.updateSelectedRules(rules);
            } else {
                const res = await this.api(
                    "POST",
                    ROUTES.rulesStore(this.selectedComp.id),
                    this.ruleForm,
                );
                this.updateSelectedRules([
                    ...this.selectedComp.rules,
                    res.rule,
                ]);
            }
            this.showRuleModal = false;
            this.showToast(
                this.editRule ? "Rule diperbarui" : "Rule ditambahkan",
            );
        },
        ...
        async deleteRule(rule) {
            ...
            this.updateSelectedRules(
                this.selectedComp.rules.filter((r) => r.id !== rule.id),
            );
        ...
        async saveComp() {
            ...
            const motorTypeIds = this.addCompForm.motorTypes
                .map((slug) => {
        ...
        async runTest() {
            const res = await this.api(
                "POST",
                ROUTES.test(this.selectedComp.id),
                this.testInputs,
            );
            this.testResult = res;
        },
