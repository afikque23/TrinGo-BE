...
<div x-show="showAddServiceModal" 
     x-cloak
     @click.away="showAddServiceModal = false"
    ...
    <div @click.stop class="bg-[#111111] border border-[#1E2939] rounded-[14px] w-full max-w-md">
        ...
        <div class="px-6 py-4 border-b border-[#1E2939]">
            <div class="flex items-center justify-between">
                ...
                <button @click="showAddServiceModal = false" class="text-[#99A1AF] hover:text-white transition-colors">
        ...
        <!-- Modal Content -->
        <form action="{{ route('admin.filters.service-types.store') }}" method="POST" class="p-6">
            @csrf
            <div class="space-y-4">
                <!-- Nama Jenis Service -->
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-[#1E2939]">
                <button type="button" @click="showAddServiceModal = false" class="px-4 h-[38px] bg-[#1A1A1A] border border-[#364153] text-white text-sm rounded-[10px] hover:bg-[#2A2A2A] transition-colors" style="font-family: Arial, sans-serif;">
                    Batal
                </button>
