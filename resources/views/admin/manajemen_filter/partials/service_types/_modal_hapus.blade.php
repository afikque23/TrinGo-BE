<!-- Modal Hapus Jenis Service -->
<div x-show="showDeleteServiceModal" 
     x-cloak
     @click.away="showDeleteServiceModal = false"
    ...
    <div @click.stop class="bg-[#111111] border border-[#1E2939] rounded-[14px] w-full max-w-md">
        ...
        <div class="px-6 py-4 border-b border-[#1E2939]">
            <h3 class="text-lg font-bold text-white" style="font-family: Arial, sans-serif;">Hapus Jenis Service</h3>
        </div>
        ...
        <div class="p-6">
            <div class="flex gap-4 mb-6">
                ...
                <div>
                    <p class="text-sm text-white mb-2" style="font-family: Arial, sans-serif;">
                        Apakah Anda yakin ingin menghapus jenis service <span class="font-bold" x-text="selectedType ? selectedType.name : ''"></span>?
                    </p>
                    <template x-if="selectedType && selectedType.services_count > 0">
                        <p class="text-sm text-[#FB2C36]" style="font-family: Arial, sans-serif;">
                            Jenis service ini masih digunakan oleh <span class="font-bold" x-text="selectedType.services_count"></span> data service dan tidak dapat dihapus.
                        </p>
                    </template>
                    <template x-if="!selectedType || selectedType.services_count === 0">
            ...
            <div class="flex items-center justify-end gap-3">
                <button type="button" @click="showDeleteServiceModal = false" class="px-4 h-[38px] bg-[#1A1A1A] border border-[#364153] text-white text-sm rounded-[10px] hover:bg-[#2A2A2A] transition-colors" style="font-family: Arial, sans-serif;">
                    Batal
                </button>
                <template x-if="selectedType && selectedType.services_count === 0">
                    <form :action="`{{ url('admin/filters/service-types') }}/${selectedType ? selectedType.id : ''}`" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
