...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
    ...
    <div class="flex-shrink-0">
        <button @click="showAddReminderModal = true" class="w-full sm:w-auto px-4 py-3 bg-[#6B7C4F] text-white text-sm rounded-[10px] hover:bg-[#5A6A40] transition-colors flex items-center justify-center gap-2" style="font-family: Arial, sans-serif;">
...
<div class="block md:hidden space-y-3">
    ...
    <div class="bg-[#111111] border border-[#1E2939] rounded-[14px] p-4">
        <div class="flex items-start justify-between gap-3 mb-3">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <span class="text-sm font-medium text-white truncate" style="font-family: Arial, sans-serif;">{{ $option['label'] }}</span>
            </div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
                <div class="w-2 h-2 rounded-full {{ $option['is_active'] ? 'bg-[#6B7C4F]' : 'bg-[#4A5565]' }}"></div>
                <span class="text-xs text-[#D1D5DC]" style="font-family: Arial, sans-serif;">{{ $option['is_active'] ? 'Aktif' : 'Nonaktif' }}</span>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-2 mb-3">
            <div class="bg-[#0A0A0A] rounded-[8px] px-3 py-2">
                <p class="text-[10px] text-[#4A5565] uppercase tracking-wider mb-0.5" style="font-family: Arial, sans-serif;">Nilai</p>
                <p class="text-xs font-medium text-[#D1D5DC]" style="font-family: Arial, sans-serif;">{{ $option['value'] }}</p>
            </div>
            <div class="bg-[#0A0A0A] rounded-[8px] px-3 py-2">
                <p class="text-[10px] text-[#4A5565] uppercase tracking-wider mb-0.5" style="font-family: Arial, sans-serif;">Unit</p>
                <span class="inline-flex items-center px-2 py-0.5 rounded-[4px] text-xs font-medium
                    @if($option['unit'] === 'km') bg-[#6B7C4F]/20 text-[#6B7C4F]
                    @else bg-[#3B82F6]/20 text-[#3B82F6] @endif">
                    @if($option['unit'] === 'km') Kilometer
                    @elseif($option['unit'] === 'days') Hari
                    @elseif($option['unit'] === 'weeks') Minggu
                    @elseif($option['unit'] === 'months') Bulan
                    @elseif($option['unit'] === 'years') Tahun
                    @endif
                </span>
            </div>
            <div class="bg-[#0A0A0A] rounded-[8px] px-3 py-2">
                <p class="text-[10px] text-[#4A5565] uppercase tracking-wider mb-0.5" style="font-family: Arial, sans-serif;">Jenis</p>
                <p class="text-xs text-[#D1D5DC]" style="font-family: Arial, sans-serif;">{{ $option['unit'] === 'km' ? 'Jarak' : 'Waktu' }}</p>
            </div>
            <div class="bg-[#0A0A0A] rounded-[8px] px-3 py-2">
                <p class="text-[10px] text-[#4A5565] uppercase tracking-wider mb-0.5" style="font-family: Arial, sans-serif;">Dibuat</p>
                <p class="text-xs text-[#99A1AF]" style="font-family: Arial, sans-serif;">{{ \Carbon\Carbon::parse($option['created_at'])->format('d M Y') }}</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <button @click="openEditReminder({{ json_encode($option) }})" class="flex-1 h-9 bg-[#1A1A1A] rounded-[8px] hover:bg-[#2A2A2A] transition-colors flex items-center justify-center gap-1.5" title="Edit">
            ...
            <form method="POST" action="{{ route('admin.filters.reminder-options.toggle-status', $option['id']) }}" class="flex-1">
                @csrf
                @method('PATCH')
                <button type="submit" class="w-full h-9 rounded-[8px] hover:opacity-80 transition-all flex items-center justify-center gap-1.5 {{ $option['is_active'] ? 'bg-[#6B7C4F]' : 'bg-[#FB2C36]' }}" title="Toggle Status">
                    ...
                    <span class="text-xs text-white" style="font-family: Arial, sans-serif;">{{ $option['is_active'] ? 'Nonaktifkan' : 'Aktifkan' }}</span>
                </button>
            </form>
            <button @click="openDeleteReminder({{ json_encode($option) }})" class="h-9 px-3 bg-[#FB2C36] rounded-[8px] hover:bg-[#E01B25] transition-colors flex items-center justify-center" title="Delete">
    ...
    @empty
    <div class="bg-[#111111] border border-[#1E2939] rounded-[14px] px-4 py-12 text-center">
        ...
        <table class="w-full table-fixed">
            ...
            <thead class="bg-[#0A0A0A] border-b border-[#1E2939]">
                <tr>
                    ...
                    <th class="px-4 py-5 text-center"><span class="text-xs font-normal uppercase tracking-wider text-[#99A1AF]" style="font-family: Arial, sans-serif;">Nilai</span></th>
                    <th class="px-4 py-5 text-left"><span class="text-xs font-normal uppercase tracking-wider text-[#99A1AF]" style="font-family: Arial, sans-serif;">Unit</span></th>
                    <th class="px-4 py-5 text-left"><span class="text-xs font-normal uppercase tracking-wider text-[#99A1AF]" style="font-family: Arial, sans-serif;">Jenis</span></th>
                    ...
                    <th class="px-4 py-5 text-left"><span class="text-xs font-normal uppercase tracking-wider text-[#99A1AF]" style="font-family: Arial, sans-serif;">Dibuat</span></th>
